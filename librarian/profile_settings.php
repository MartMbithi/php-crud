<?php
     ...
     if(isset($_POST['update_librarian']))
    {
            ...
            if (isset($_POST['librarian_phone_number']) && !empty($_POST['librarian_phone_number'])) {
                ...
                $postQuery="UPDATE librarians SET librarian_bio=?, librarian_name =?, librarian_number =?, librarian_email =?, librarian_phone_number =?, librarian_address =?, librarian_profile_picture =? WHERE librarian_login_id =?";
                $postStmt = $mysqli->prepare($postQuery);
                //bind paramaters
                $rc=$postStmt->bind_param('ssssssss', $librarian_bio, $librarian_name, $librarian_number, $librarian_email, $librarian_phone_number, $librarian_address, $librarian_profile_picture,  $login_id);
                $postStmt->execute();
                //declare a varible which will be passed to alert function
                if($postStmt)
                {
                    $success = "Librarian Account Updated" && header("refresh:1; url=profile.php");
                }
                else 
                {
                                    ...
                                    <form method="POST" enctype="multipart/form-data">
                                        <div class="form-row mb-4">
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <input type="text" name="librarian_number" readonly value="<?php echo $lib->librarian_number;?>" class="form-control">
                                            </div>
                                        </div>
                                        <div class="form-row mb-4">
                                            <div class="form-group col-md-6">
